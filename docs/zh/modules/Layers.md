...
<p align="center"><img :src="$withBase('/layer-manager.png')" alt="GrapesJS - 层管理器"/></p>

层管理器 (Layer Manager) 模块负责将您的 [组件 (Components)] 作为树状结构进行管理和显示。

::: warning
本指南适用于 GrapesJS v0.19.5 或更高版本
:::
...
要更改默认配置，您需要在主配置对象中传递 `layerManager` 选项。
...
您可以在此处查看可用配置选项的完整列表：[层管理器配置 (Layer Manager Config)](https://github.com/GrapesJS/grapesjs/blob/master/src/navigator/config/config.ts)

层 (Layers) 是组件的直接表示，因此它们只有在组件加载到编辑器后才可用（例如，您可能从远程端点加载项目数据）。

在您的配置中，您可以更改层的全局行为（例如，使所有层都不可排序），并决定哪个组件层应用作根 (root)。
...
这些配置主要针对 GrapesJS 核心提供的默认用户界面 (UI)，如果您需要对层的树状结构进行更多控制，可以阅读下面的 [自定义](#customization) 部分。

## 程序化用法

如果您需要以编程方式管理层，可以使用其 [API][Layers API]。

## 自定义

通过使用 [Layers API][Layers API]，您可以用自己的实现替换默认 UI。

您所要做的就是向编辑器表明您打算使用自定义 UI，然后订阅一些事件，以便正确更新您的 UI。
...
// 使用此事件将您的 UI 附加到 GrapesJS 提供的默认容器中。
// 如果您不依赖核心面板 (core panels) 并决定将 UI 放置在其他位置，
// 则可以跳过此事件。
editor.on('layer:custom', (props) => {
  // props.container (HTMLElement) - 您可以将 UI 附加到的默认元素
});

// 当根层 (root layer) 更改时触发。
...
有关可用事件的完整列表，您可以在[此处](/api/layer_manager.html#available-events)查看。
